...
diffusion.load_state_dict(torch.load('/home/wangpengkai/node/self_diffusion_learn/models/cylinder/model-10.pt', map_location='cuda:1')['model'])
...
for i in range(4):
    ...
    ax.set_xlabel('x')  
    ax.set_ylabel('y') 
    im = ax.imshow(sampled_images[i, 0], cmap="RdBu", aspect='auto')
    ax.set_xticks([])
    ax.set_yticks([])
...
train_data_dir = '/home/wangpengkai/node/self_diffusion_learn/data'
cy_file = os.path.join(train_data_dir, 'Cy_Taira.pickle')

# load grid numerical data file of f(x, y)
with open(cy_file, 'rb') as f:
    cy_box = pickle.load(f).copy()
...
for i in range(4):
    ...
    ax.set_xlabel('x')  
    ax.set_ylabel('y') 
    im = ax.imshow(standardized_data[j, 0], cmap="RdBu", aspect='auto')
    ax.set_xticks([])
    ax.set_yticks([])
    ax.spines['bottom'].set_linewidth(0)
    ax.spines['top'].set_linewidth(0)
...
plt.savefig("ddpm_combined_cylinder_100.png", bbox_inches='tight', dpi=400)