...
diffusion.load_state_dict(torch.load('/home/wangpengkai/node/self_diffusion_learn/models/darcy/model-10.pt', map_location='cuda:1')['model'])
...
for i in range(4):
    ...
    ax.set_xlabel('x')  
    ax.set_ylabel('y') 
    im = ax.imshow(sampled_images[i, 0], cmap="jet", aspect='auto')
    ax.set_xticks([])
    ax.set_yticks([])
...
for i in range(4):
    ...
    ax.set_xlabel('x')  
    ax.set_ylabel('y') 
    im = ax.imshow(standardized_data[j, 0], cmap="jet", aspect='auto')
    ax.set_xticks([])
    ax.set_yticks([])
    ax.spines['bottom'].set_linewidth(0)
    ax.spines['top'].set_linewidth(0)
...
plt.savefig("ddpm_combined_darcy_250.png", bbox_inches='tight')