...
diffusion.load_state_dict(torch.load('/home/wangpengkai/node/self_diffusion_learn/models/airfoil/model-10.pt', map_location='cuda:1')['model'])
...
index = np.random.choice(1000, size=4, replace=False)
for i in range(4):
    j = index[i]
    ax = axes[1, i]
    ax.set_xlabel('x')  
...
plt.savefig("ddpm_combined_airfoil_200.png", bbox_inches='tight', dpi=400)